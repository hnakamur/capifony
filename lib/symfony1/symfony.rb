namespace :symfony do
  ...
  task :default do
    ...
    stream "cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony #{task_arguments}"
  end
  ...
  task :cc do
    run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{try_sudo} #{php_bin} ./symfony cache:clear'"
    run "#{try_sudo} chmod -R g+w #{latest_release}/#{symfony_dir}/cache"
  end

  desc "Creates symbolic link to symfony lib in shared"
  task :create_lib_symlink do
    prompt_with_default(:version, symfony_version)
    symlink_path = "#{latest_release}/#{symfony_dir}/lib/vendor/symfony"
  ...
  namespace :configure do
    ...
    task :database do
      ...
      if current_logger_level >= Capistrano::Logger::DEBUG
        logger.debug %(executing "cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony configure:database '#{dsn}' '#{db_username}' ***")
        self.logger.level = Capistrano::Logger::INFO
      end

      stream "cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony configure:database '#{dsn}' '#{db_username}' '#{db_password}'"
  ...
  namespace :project do
    ...
    task :disable do
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony project:disable #{symfony_env_prod}'"
    end

    desc "Enables an application in a given environment"
    task :enable do
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony project:enable #{symfony_env_prod}'"
    end

    desc "Fixes symfony directory permissions"
    task :permissions do
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony project:permissions'"
    end
    ...
    task :optimize do
      ...
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony project:optimize #{application}'"
    end

    desc "Clears all non production environment controllers"
    task :clear_controllers do
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony project:clear-controllers'"
    end
    ...
    task :send_emails do
      ...
      stream "cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony project:send-emails --message-limit=#{message_limit} --time-limit=#{time_limit} --env=#{symfony_env_prod}"
    end
  ...
  namespace :plugin do
    ...
    task :publish_assets do
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony plugin:publish-assets'"
    end
  end
  ...
  namespace :log do
    ...
    task :clear do
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony log:clear'"
    end
    ...
    task :rotate do
      ...
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony log:rotate #{application} #{symfony_env_prod}'"
    end
  end
  ...
  namespace :tests do
    ...
    task :all do
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony test:all'"
    end
    ...
    task :functional do
      ...
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony test:functional #{application}'"
    end

    desc "Launches unit tests"
    task :unit do
      run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony test:unit'"
    end
  end
