namespace :symfony do
  namespace :propel do
    ...
    task :build_classes do
      run "#{try_sudo} #{php_bin} #{latest_release}/#{symfony_dir}/symfony propel:build --model --env=#{symfony_env_prod}"
      run "#{try_sudo} #{php_bin} #{latest_release}/#{symfony_dir}/symfony propel:build --forms --env=#{symfony_env_prod}"
      run "#{try_sudo} #{php_bin} #{latest_release}/#{symfony_dir}/symfony propel:build --filters --env=#{symfony_env_prod}"
    end
    ...
    task :build_all do
      if Capistrano::CLI.ui.agree("Do you really want to rebuild #{symfony_env_prod}'s database? (y/N)")
        run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony propel:build --sql --db --no-confirmation --env=#{symfony_env_prod}'"
      end
    end
    ...
    task :build_all_and_load do
      if Capistrano::CLI.ui.agree("Do you really want to rebuild #{symfony_env_prod}'s database and load #{symfony_env_prod}'s fixtures? (y/N)")
        run "#{try_sudo} sh -c 'cd #{latest_release}/#{symfony_dir} && #{php_bin} ./symfony propel:build --sql --db --and-load --no-confirmation --env=#{symfony_env_prod}'"
      end
    end
